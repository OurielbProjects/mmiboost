        ...
        <div className="flex items-center justify-between gap-4 p-5 sm:p-6">
          ...
          <div
            className={`flex-shrink-0 w-8 h-8 rounded-full flex items-center justify-center transition-all duration-300 ${
              open ? 'bg-blue-500/20 text-blue-400 rotate-180' : 'bg-white/5 text-gray-400'
            }`}
          >
    ...
    <section id="faq" className="section bg-[#080810] relative overflow-hidden">
      ...
      <div className="container-xl relative z-10">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-start">
          {/* Left */}
          <AnimatedSection>
            <SectionBadge icon="💬">{f.badge}</SectionBadge>
            <h2 className="font-display text-4xl md:text-5xl font-bold text-white mb-5">
              {f.title1}{' '}
              <span className="gradient-text">{f.titleGradient}</span>
            </h2>
            ...
            >
              ...
              <h3 className="font-semibold text-white mb-2">{f.contactTitle}</h3>
              <p className="text-sm text-gray-400 mb-4">{f.contactDesc}</p>
              <a href="mailto:contact@mmiboost.com" className="btn-primary text-sm py-2.5 px-6 inline-flex">
                <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
